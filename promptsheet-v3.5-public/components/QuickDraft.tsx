...
export function QuickDraft({
    ...
    const handleClear = useCallback(() => {
        ...
        // If the current actual text differs from the last saved history state,
        // force it onto the stack first so we can undo directly back to it.
        if (historyRef.current[historyIndexRef.current] !== localContent) {
            historyRef.current = [
                ...historyRef.current.slice(0, historyIndexRef.current + 1),
                localContent,
            ].slice(-50);
    ...
    // +New Draft handler
    const hasContent = localTitle.trim().length > 0 || localDescription.trim().length > 0 || localContent.trim().length > 0;

    const handleNewDraft = useCallback(() => {
        if (!hasContent) return;

        const isFullySaved = libraryItems.some(
            (item) =>
    ...
    }, [hasContent, libraryItems, localTitle, localDescription, localContent, onSave]);
    ...
    return (
        <section className="px-5 py-4">
            ...
            >
                ...
                <div className="relative">
                    ...
                    <button
                        ...
                        className="absolute top-2 right-7 text-slate-400 hover:text-slate-600 dark:text-zinc-300 dark:hover:text-zinc-100 transition-colors outline-none focus:outline-none focus-visible:outline-none"
                        title="Clear draft"
                    >
                    ...
                    <button
                        ...
                        className="absolute bottom-2 right-7 text-slate-400 hover:text-slate-600 dark:text-zinc-300 dark:hover:text-zinc-100 transition-colors outline-none focus:outline-none focus-visible:outline-none"
                ...
                {/* Navigation arrows + New Draft + Copy Draft + Save Draft */}
            ...
            {/* Clear confirmation modal (New Draft) */}
            {showClearConfirm && (
                <ConfirmModal
            ...
            {/* Refresh/clear body confirmation modal */}
            {showRefreshConfirm && (
                <ConfirmModal
                    title="Clear body content?"
                    message="This will clear the body field. You can undo this action afterwards."
                    confirmLabel="Clear"
                    onConfirm={() => {
                        handleClear();
